...
public abstract class AbstractJsonRestResource<T> implements IResource {
	...
	for (int i = 0; i < argsClasses.length; i++) {
	    ...
	    if (paramValue == null)
		throw new WicketRuntimeException(
			"Could not find a value for the " + (i +1)
    ...
    private Object extractParameterValue(int i, Method targetMethod,
	    Class<?> argClass, PageParameters pageParameters) {
	Object paramValue = null;
	Annotation[][] parametersAnnotations = targetMethod.getParameterAnnotations();
	
	if (ReflectionUtils.isParameterAnnotatedWith(i, targetMethod,
		JsonBody.class))
	    paramValue = extractObjectFromBody(argClass);
	else if (ReflectionUtils.isParameterAnnotatedWith(i, targetMethod,
		QueryParam.class))
    ...
    private Object extractParameterFromHeader(Annotation[] parameterAnnotations,
	    Class<?> argClass) {

	HeaderParam headerParam = ReflectionUtils.findAnnotation(parameterAnnotations, HeaderParam.class);
	...
	QueryParam queryParam = ReflectionUtils.findAnnotation(parameterAnnotations, QueryParam.class);
	...
	try {
	    ...
	    throw new RuntimeException(
		    "Error retrieving a constructor with a string parameter.",e);
	}
    }
